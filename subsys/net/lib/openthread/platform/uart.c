...
OT_UART_DEFINE(ot_uart, CONFIG_OPENTHREAD_NCP_UART_TX_RING_BUFFER_SIZE)

#define RX_FIFO_SIZE 128

static void uart_rx_handle(/*const struct openthread_uart *ot_uart*/)
{
	u8_t *data;
	u32_t len;
	u32_t rd_len;
...
static void uart_tx_handle(/*struct openthread_uart *ot_uart*/)
{
	u32_t len;
...
otError otPlatUartEnable(void)
{
	...
	ret = uart_line_ctrl_get(
		ot_uart.dev, UART_LINE_CTRL_BAUD_RATE,
		&baudrate);
	if (ret) {
